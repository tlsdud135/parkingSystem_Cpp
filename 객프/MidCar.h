#pragma once
#ifndef __MID_CAR_H__
#define __MID_CAR_H__

#include "Car.h"
//추상클래스 상속하는 클래스
class MidCar :public Car
{
private:
	const int BasicRate = 750;//기본요금(20분(초)간)
	const int Extra = 350;//추가요금(10분(초)마다)
	void timeCalc() {
...
public:
	MidCar(int carSize, string carName) :Car(carSize, carName) {}
	virtual void Out()
	{
		timeCalc();//시간 계산
	...
	virtual void ShowCarInfo() {
		...
		cout << "차 종: " << "준중형차, 중형차" << endl;//중형차 클래스이므로
